  ...
  while (stack.length > 0) {
    ...
    for (let index = entries.length - 1; index >= 0; index -= 1) {
      ...
      stack.push({
        ...
        depth: frame.depth + 1,
        role: roleForObjectChild(frame.role, key),
      });
    }
  }
...
function inspectArray(
  ...
  for (const key of keys) {
    ...
    if (index === undefined || index >= value.length) {
      registerString(statistics, limits, key, path);
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    try {
      ...
      throw nonJson(
        ...
        "array item could not be inspected",
        cause,
      );
    }
    if (!descriptor) {
...
function inspectObject(
  ...
  return names.map((key) => {
    ...
    try {
      ...
      throw nonJson(
        appendKey(path, key),
        "object property could not be inspected",
        cause,
      );
    }
    if (!descriptor) {
      throw nonJson(
        appendKey(path, key),
        "object property disappeared during inspection",
      );
    }
